...
def poses_avg_back(poses):

    hwf = poses[0, :3, -1:]

    center = np.concatenate([poses[:, :2, 3].mean(0), poses[:, 2:3, 3].max(0)], axis=0)
    vec2 = normalize(poses[:, :3, 2].sum(0))
    up = poses[:, :3, 1].sum(0)
    c2w = np.concatenate([viewmatrix(vec2, up, center), hwf], 1)
...
def load_llff_data(basedir, factor=8, width=None, height=None,
                   recenter=True, rerotate=True, move_back=False,
    ...
    if recenter:
        ...
        if move_back:
            poses = recenter_poses_back(poses)
